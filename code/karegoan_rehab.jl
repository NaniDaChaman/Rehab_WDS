...
function check_feasibility(model)
	...
	if termination_status(model)==INFEASIBLE::TerminationStatusCode
        ...
        write_model(model,"karegon_rehab.lp")
...
data = readxl("input/karegon_scheme.xls", "Sheet1!A126:F133")
#println(data)
T=length(data[:,1])
...
#objective function
@objective(rehab_problem,Min,sum(C_l.*l)+sum(CP*elc_c.*pe)+sum(UN_n.*z+CT_n.*e) + DF*sum(K.*dp_t)-prev_cl) # opex cost will be sum(K.*d_bar) and with 
pipe_cost=@expression(rehab_problem,sum(C_l.*l))
tank_cost=@expression(rehab_problem,sum(UN_n.*z+CT_n.*e))
pump_capital=@expression(rehab_problem,sum(CP*pe))
...
M_d=2.4567*maximum([flow_p;flow_s])
#throw(DomainError("Stoped by user"))
...
for i =1:NL
    ...
    @constraint(rehab_problem,dp_t[(i-1)*T+1:i*T].<=flow_pt[(i-1)*T+1:i*T].*f[i]+flow_st[(i-1)*T+1:i*T].*(1-f[i]))
    #@constraint(rehab_problem,M_d*(ones(T)-x_t[(i-1)*T+1:i*T]).>=flow_pt[(i-1)*T+1:i*T].*f[i]+flow_st[(i-1)*T+1:i*T].*(1-f[i])-dp_t[(i-1)*T+1:i*T])
...
title_text="Rehabilitation of the Karegoan Redesign Scheme"
fname="karegoan_rehab"
...
PlotlyJS.savefig(tank_schedule,"output\\Karegaon Tank Schedule.png")
