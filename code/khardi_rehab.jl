...
function save_network(title_text,filename,demand_nodes,ref_node,tank_loc,tank_type,T_min,T_max,UP,pipe_seg,link_list,l_star,possible_pipes,df_t,delta,atank_star)
    file_text="[TITLE]\n$(title_text)"
    ...
    for (k,t) in enumerate(tank_loc)
        tanks*="tank$(k)\t$(demand_nodes[k].elevation)\t$(T_min)\t$(T_min)\t$(T_max)\t$(UP[k]/T_max)\t$(0)\t\t;\n"
    end
    file_text*=tanks
    ...
    for (i,k) in pipe_seg
        ...
        if n in tank_loc
            ...
            pipes*="$(NL+i)\t$(link_list[i].start_node)\ttank$(tank_k)\t$(l_star[i][k])\t$(possible_pipes[k].diameter)\t$(possible_pipes[k].roughness)\t$(0)\tOpen\t;\n"
            pipes*="$(2NL+i)\ttank$(tank_k)\t$(link_list[i].end_node)\t$(l_star[i][k])\t$(possible_pipes[k].diameter)\t$(0)\t$(0)\tOpen\t;\n"
        end
            pipes*="$(i)\t$(link_list[i].start_node)\t$(link_list[i].end_node)\t$(l_star[i][k])\t$(possible_pipes[k].diameter)\t$(possible_pipes[k].roughness)\t$(0)\tOpen\t;\n"
    end
    file_text*=pipes
    ...
    patterns*="1"*reduce(*,map(x->"\t$(x)",df_t))*"\n"
    file_text*=patterns
    ...
    for n in tank_loc
        ...
        i=P.link_id
        for t=1:T
            controls*="Link $(i) $(status(atank_star[n][t])) AT TIME $(string(Time(t*3%24,0)))\n"
            controls*="Link $(NL+i) $(opps_status(atank_star[n][t])) AT TIME $(string(Time(t*3%24,0)))\n"
        end
    end
    file_text*=controls
    ...
    times="\n[TIMES]\n"
    times*="Duration\t24:00\nHydraulic Timestep\t$(string(delta))\nQuality Timestep\t$(string(delta))\nPattern Timestep\t$(string(delta))\nPattern Start\t0:00\nReport Timestep\t$(string(delta))\nReport Start\t0:00\n"
    file_text*=times
...
function check_feasibility(model)
	...
	if termination_status(model)==INFEASIBLE::TerminationStatusCode
        ...
        write_model(model,"khardi_rehab.lp")
    ...
    elseif (termination_status(model)==OPTIMAL::TerminationStatusCode)
        ...
        global hl_star=value.(hl_t)
        global atank_star=value.(atank_t)
        global es_star=value.(es)
        global e_star=value.(e)
        tank_d(e_star)
        global s_star=value.(s)
        ...
        global hbar_star=[hbar_star[(n-1)*T+1:n*T] for n=1:N]
        global hl_star = [hl_star[(i-1)*T+1:i*T] for i=1:NL]
        global atank_star = [atank_star[(n-1)*T+1:n*T] for n=1:N]
        global es_star=[es_star[(n-1)*NL+1:n*NL] for n=1:N]
        tank_location(es_star)
        global s_star=[s_star[(n-1)*N+1:n*N] for n=1:N]
        global pe_star=value.(pe)
        global x_star=value.(x_t)
        global x_star = [x_star[(i-1)*T+1:i*T] for i=1:NL]
        global d_star = value.(d)
        global d_star=[d_star[(n-1)*T+1:n*T] for n=1:N]
...
data = readxl("input/khardi scheme.xls", "General!D15:D18")
...
data = readxl("input/khardi scheme.xls", "General!D14:D18")
...
# reading nodes
data = readxl("input/khardi scheme.xls", "General!A24:E34")
#println(data)
N=11
...
for i = 1:number_of_rows 
    d1=demand_node(data[i,1],data[i,3],data[i,4],data[i,5])
...
#readin pipes
data = readxl("input/khardi scheme.xls", "General!A52:C64")
...
# reading links
data = readxl("input/khardi scheme.xls", "General!A38:E47")
NL=10#number of links = number of rows
...
println("link structure is  : $(link_structure[11,:])")
...
#tank general data 
data = readxl("input/khardi scheme.xls", "General!D69:D75")
SH::Float64=data[2]
tank_cp::Float64=data[3]
T_max::Float64=data[4]
T_min::Float64=10
#tank data 
data = readxl("input/khardi scheme.xls", "General!A80:D89")
NE=10
...
data = readxl("input/khardi scheme.xls", "General!D93:D103")
PP_min=data[2]
eff=data[11]
CP=data[4]
EP=data[5]
...
data = readxl("input/khardi scheme.xls", "General!A142:F149")
#println(data)
T=length(data[:,1])
...
for i=1:T
    ...
    td1 = time_period(Int(data[i,1]),data[i,2],data[i,3],data[i,4],data[i,5])
    time_slices[i]=td1
end
...
elc_c=18.67*ones(NL)
#electricity prices 
pie=[t.elec_price/3600 for t in time_slices]
#electricity consumption coefficient 
ph=elc_c.*(eff/(g*water_density))
...
# @constraint(rehab_problem,l[index_l].==L[index_i])
# prev_cl=@expression(rehab_problem,sum(C_l[index_l].*l[index_l]))

# println("\nPrevious link feasibility")
# check_feasibility(rehab_problem)

#objective function
@objective(rehab_problem,Min,sum(C_l.*l)+sum(CP*elc_c.*pe)+sum(UN_n.*z+CT_n.*e) + DF*sum(K.*dp_t)) # opex cost will be sum(K.*d_bar) and with 
pipe_cost=@expression(rehab_problem,sum(C_l.*l))
tank_cost=@expression(rehab_problem,sum(UN_n.*z+CT_n.*e))
pump_capital=@expression(rehab_problem,sum(CP*p))
pump_opex=@expression(rehab_problem,sum(PH*EP*DF*p_p)+sum(SH*EP*DF*p_s))
total_cost=0
...
path=S_n(4,link_structure,11)
#println("\nPath from 8 to 7 is $(path)\n")
#navigating the path


println("Path from 8->7 is : $(get_curr_path(11,4,path))")
...
M_p=2*maximum([maximum([L'*HL_p;L'*HL_s]),H_r])
...
#pressure constraint , tank filling 1 : T_max.-M_h*(ones(T)-b[(m-1)*T+1:m*T]).<=h_bar[(m-1)*T+1:m*T]
    ...
    #@constraint(rehab_problem,E_n[m].-M_p*(ones(T)-b_t[(m-1)*T+1:m*T]).+tl_t[(m-1)*T+1:m*T].>=hbar_t[(m-1)*T+1:m*T])
#pressure constraint , tank filling 2 : l[(m-1)*T+1:m*T]+M_h*(b[(m-1)*T+1:m*T]).<=h_bar[(m-1)*T+1:m*T]
    ...
    @constraint(rehab_problem,T_min.+M_p*(b_t[(m-1)*T+1:m*T]).+E_n[m].<=hbar_t[(m-1)*T+1:m*T])
...
title_text="Rehabilitation of the Khardi Redesign Scheme"
fname="khardi_rehab"
save_network(title_text,fname,demand_nodes,ref_node,tank_loc,tank_type,T_min,T_max,UP,pipe_seg,link_list,l_star,possible_pipes,df_t,delta,atank_star)
m2=transpose(map(x-> x!=0 ,hcat(atank_star[tank_loc]...)))
trace1=heatmap(z=100*m2,colorscale="Blues",showscale=false,x=["t1","t2","t3","t4","t5","t6","t7","t8"],y=string.(tank_loc))
layout=Layout(title="Tank Operation Schedule",
xaxis_title="Time Slices",
yaxis_title="Tank Node Location",)
tank_schedule=plot(trace1,layout)
PlotlyJS.savefig(tank_schedule,"output\\Khardi Tank Schedule.png")
total_cost=value.(objective_function(rehab_problem))
