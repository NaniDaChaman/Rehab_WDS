...
println("Headloss for each possibble assignment : $(HL[1,:]) \n its size $(size(HL))")
...
#binary variable
@variable(pipe_model,x[1:NP*NL],Bin)

#objective
@objective(pipe_model,Max,sum(LC.*x))

#length constraint
for j=1:NL
		@constraint(pipe_model,sum(x[1+(j-1)*NP:(j)*NP])==1)
end
...
for n=1:N
	...
	@constraint(pipe_model,P_n[n]<=H_r-E_n[n]-sum([HL_n.*x[index_n];0]))
end
optimize!(pipe_model)
x_star=reshape(value.(x),NL,NP)
println(x_star)
#print(pipe_model)
